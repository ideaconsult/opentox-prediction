	...
	public static void main(String[] args) throws ServiceException{
		...
		if (args[0].equals("fetch")) {
			...
			try {
				...
				ModelResponse modelResponse = stub.fetchModel(taskID);
			...
			try {
				...
				if (args[0].equals("post")) {
					modelResponse = stub.postModel(modelID, sdf);
				}

				else if (args[0].equals("postm")) {

					modelResponse = stub.postModelSingleSDF(modelID, sdf[0]);
				}
...
ModelServiceStub stub  = new ModelServiceStub();

ApplyModel amData = new ApplyModel();

amData.setModelId(modelID);

amData.setSdf(sdf);

ApplyModelResponse amResponse = stub.applyModel(amData);

System.out.println("STATUS:                " + amResponse.get_return().getStatus());

ModelResponse modelResponse = amResponse.get_return();

System.out.println("PREDICTED VALUE:       " + modelResponse.getPredictionValue());

System.out.println("PREDICTION ACCURACY:   " + modelResponse.getAccuracy());

System.out.println("DESCRIPTION URL:       " + modelResponse.getModelDescriptionUrl());
